...
function updateDB(coll, data) {
  const _id = data._id
  delete data._id
  return db.collection(coll).doc(_id).update({
    data
  })
}
...
async function putCard(cardId) {
  return;
  const box = (await db.collection("boxs").where({
    "name": "develop"
  }).get()).data[0]
  ...
  switch (type) {
    case "FOUND":
      ...
      if (res.data && res.data.length) {
        ...
        //TODO 服务通知
        if (user.lastPost) {
          const post = (await db.collection("posts").where({
            _id: user.lastPost
          }).get()).data[0]
          post.status = "已找到"
          post.date = Date().now();
    ...
    case "LOSS":
      ...
      if (res.data && res.data.length) {
        ...
        //TODO 服务通知
        res = (await db.collection("posts").where({
          rawData: {
            cardId: user.cardId
          }
        }).get())
    ...
    case "CANCEL":
      ...
      if (res.data && res.data.length) {
        const user = res.data[0]
        if (user.status == "已挂失") {} else if (user.status == "已找到") {
